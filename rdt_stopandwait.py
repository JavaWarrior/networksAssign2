...
class rdt_stopandwait(rdt):
	...
	def rdt_send(self, msg):
		sent = 0
		max_trials_num = max(10, len(msg) / packet_data_size * 3)
		trials = 0
		data = self.make_pkt(msg, self.send_seq_num)
		while 1:
			# print(sent)
	...
	def rdt_receive(self):
		...
		while(1):
			ready = select.select([self.self_socket], [], [], self.timeout_val)
			if(ready[0]):
				data,self.to_add = self.self_socket.recvfrom((packet_data_size+self.header_size)*2)
				print("rdt")
				print(data)
